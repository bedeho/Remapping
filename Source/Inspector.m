...
%  Remapping
%
%  Created by Bedeho Mender on 03/07/13.
%  Copyright 2013 OFTNAI. All rights reserved.
%

function Inspector(netDir)

    % Experiment name
    if nargin < 1,
        netDir = 'prewired-tuning';
    end
    ...
    % Load stimuli
    disp('Loading activity stimuli...');
    
    stimuli  = cell(1, length(stimulinames));
    activity = cell(1, length(stimulinames));
    analysis = cell(1, length(stimulinames));
    
    for i = 1:length(stimulinames),
        stimuli{i}    = load([STIMULI_FOLDER stimulinames{i} filesep 'stim.mat']);
        activity{i}   = load([netDir filesep 'activity-' stimulinames{i} '.mat']);
        analysis{i}   = load([netDir filesep 'analysis-' stimulinames{i} '.mat']);
    end
    
    %% R
    
    % Load locations
    %location = analysis{3}.locations
    
    % Plot neuron locations of neurons
    
    % Add mouse click capture for plot
    
    %% C
    
    % Load network (prewired?)
    
    % For each C neuron, find center off mass of synapses from V and onto S
    
    % Plot v,r in plot
    
    % Add mouse click listener
    
    % Plot extra histogram of given neurons projeciton to R.
    
    % Add drop down with different stimuli
