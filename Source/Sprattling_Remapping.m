...
function Sprattling_Remapping(simulationFolder, stimuliName, isTraining, networkfilename)
    ...
    K_to_R_weights = network.K_to_R_weights;
    ...
    K_to_R_weights_dilutionmap = network.K_to_R_weights_dilutionmap;
    ...
    % new
    K_to_R_psi      = parameters.simulation('K_to_R_psi');
    K_to_R_alpha    = parameters.simulation('K_to_R_alpha');
    ...
    for epoch=1:numEpochs,
        ...
        for period=1:numPeriods,
            ...
            for t=2:numTimeSteps, % we cannot compute anything for t==1, since this is time=0, which are the initial conditions, we have not t=-dt input data to use for this
                ...
                K_to_R_excitation = K_to_R_psi*(K_to_R_weights*K')';
                
                C_to_R_excitation = C_to_R_psi*(C_to_R_weights*C_firingrate')';
                R_activation = R_activation + (dt/R_tau)*(-R_activation + C_to_R_excitation - R_global_inhibition + K_to_R_excitation ); % + R_visual_onset, - R_background
                ...
                if isTraining,
                    ...
                    K_to_R_weights = K_to_R_weights + dt*K_to_R_alpha*(R_firingrate'*K);
                    
                    % capped
                    %C_to_R_weights = C_to_R_weights + dt*C_to_R_alpha*(R_firingrate'*C_firingrate)*(C_to_R_weights);
                    %K_to_R_weights = K_to_R_weights + dt*K_to_R_alpha*(R_firingrate'*K)*;
                    %=========================
                    ...
                    K_to_R_weights = K_to_R_weights.*K_to_R_weights_dilutionmap;
        ...
        if mod(epoch, parameters.saveNetworksAtEpochMultiples) == 0,
            ...
            save([simulationFolder filesep 'TrainedNetwork_e' num2str(epoch) '.mat'] , 'C_to_R_weights', 'K_to_R_weights', 'S_to_C_weights', 'V_to_C_weights',  'C_to_R_weights_dilutionmap', 'K_to_R_weights_dilutionmap', 'S_to_C_weights_dilutionmap', 'V_to_C_weights_dilutionmap', 'R_N', 'S_N', 'C_N');
        end
    end
    ...
    if isTraining,
        ...
        save([simulationFolder filesep 'TrainedNetwork.mat'] , 'C_to_R_weights', 'K_to_R_weights', 'S_to_C_weights', 'V_to_C_weights',  'C_to_R_weights_dilutionmap', 'K_to_R_weights_dilutionmap', 'S_to_C_weights_dilutionmap', 'V_to_C_weights_dilutionmap', 'R_N', 'S_N', 'C_N');
    end
    ...
    if saveOutput && numel(C_firing_history) > 2000000/2, % dont save if bigger than 100MB
        ...
        % also cancel extra if its size of C
        [e_m,e_n] = size(extra_history);
        if e_m == C_N,
            extra_history = [];
        end
    end
