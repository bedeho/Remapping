...
function StandAloneRemapping()
    ...
    % Visual
    headCenteredTargetLocations = [0]; % (deg)
    maxNumberOfVisibleTargets = length(headCenteredTargetLocations);
    
    targetOffIntervals{1} = [1.0 2.5];%[0.5 0.9;]; % (s) [start_OFF end_OFF; start_OFF end_OFF] <==== Make dt multiples
    ...
    V = zeros(1,R_N);
    V_history = zeros(R_N,numTimeSteps);
    V_history2 = zeros(R_N,numTimeSteps);
    ...
    S_delay_sigma = 0.2; % (s)
    %classic: S_presaccadicOffset = S_delay_sigma*randn(1,S_N);
    S_presaccadicOffset = S_delay_sigma*ones(1,S_N);
    ...
    for t=1:numTimeSteps,
        ...
        F = (saccade_time_neg_offset <= time) & (time <= saccade_time_pos_offset); % check both conditions: y-z <= x <= y+z
        ...
        % Save in history
        R_history(:,t) = R_firingrate;
        S_history(:,t) = S_firingrate;
        C_history(:,t) = C_firingrate;
        V_history(:,t)  = V;
    ...
    % Plot
    s = timeToTimeStep(saccadeTimes);
    
    
    figure('Position', [100, 100, 1049, 895]);
    
    subplot(5,2,1);
    imagesc(flipud(V_history));
    hold on;plot([s s],[ones(R_N,1) R_N*ones(R_N,1)],'r');
    colorbar
    title('V Firing');
    
    subplot(5,2,2);
    imagesc(flipud(V_history2));
    hold on;plot([s s],[ones(R_N,1) R_N*ones(R_N,1)],'r');
    colorbar
    title('V Firing');
